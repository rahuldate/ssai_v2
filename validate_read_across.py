...
def main():
    ...
    print("=== Mode: tanimoto (fingerprint similarity only, baseline) ===\n")
    for k, cutoff in [(1, 0.4), (3, 0.4), (3, 0.3), (5, 0.3)]:
        y_true, y_pred, ood, inc = loo_validate(combined, k, cutoff, similarity_mode="tanimoto")
    ...
    print("=== Mode: combined (tanimoto + structural-alert overlap) ===\n")
    for alpha in [0.7, 0.5, 0.3]:
        for k, cutoff in [(3, 0.3), (5, 0.3)]:
            y_true, y_pred, ood, inc = loo_validate(combined, k, cutoff, similarity_mode="combined", alpha=alpha)
            m = compute_metrics(y_true, y_pred)
