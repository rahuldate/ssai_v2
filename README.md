...
|---|---|---|
| DPRA 6.38 / 22.62 / 42.47% depletion bands | Verified (primary) | ECVAM DPRA Validation Study Report's own prediction-model figure |
...
drives sensitization.

### Experiment: combining Tanimoto similarity with structural-alert overlap

`compute_read_across` supports an opt-in `similarity_mode="combined"` that
blends Tanimoto fingerprint similarity with Jaccard similarity over the
Structure tab's 8 SMARTS alerts (`alpha` controls the blend weight; not
exposed in the main app UI -- see why below). Result, same 54-compound
LOO-CV:

| mode | best MCC | specificity at that setting |
|---|---|---|
| tanimoto (baseline) | -0.064 (k=5, cutoff=0.3) | 0.273 |
| combined, alpha=0.7 | -0.389 | 0.000 |
| combined, alpha=0.5 | -0.192 | 0.000 |
| combined, alpha=0.3 | -0.161 | 0.000 |

**This made things measurably worse, not better -- specificity collapses to
0.000 in every combined-mode configuration tested.** Diagnosed cause: 68% of
sensitizers and 65% of non-sensitizers in this dataset trigger zero of the 8
SMARTS alerts. The alert-similarity function scores a both-absent pair as
0.0 (deliberately, so two unrelated inert molecules don't score as "similar"
just for both lacking a reactive group) -- but since most non-sensitizer
pairs are both-absent, this drags every non-sensitizer's combined score down
uniformly, while the minority of sensitizer pairs that *do* share a
triggered alert get boosted. Net effect: nothing ever outranks a sensitizer,
so a non-sensitizer never wins a neighbor vote.

**This is not wired into the app's UI** as a result -- presenting an
empirically worse option as a live choice would be the wrong call. The code
path is implemented, tested (`tests/test_core.py`), and documented so a
future attempt at this idea starts from a known failure mode instead of
repeating it. A revised design would need to handle the both-absent case
differently and/or use a richer alert set than the current 8 simplified
SMARTS patterns before it's worth re-testing.

Re-run `validate_read_across.py` and update this section whenever the
reference data or similarity logic changes materially.
