...
def _p(text) -> str:
    """Escape text for safe use inside a ReportLab Paragraph, which parses a
    small XML/HTML-like markup. Without this, a compound name, note, or
    uploaded-CSV value containing '&', '<', or '>' crashes PDF generation."""
    return _xml_escape(str(text))


def _safe_filename(name: str) -> str:
    """Keep a filename download-safe: only letters, numbers, dash, underscore."""
...
def _section(story, styles, title, lines):
    ...
    if not lines:
        ...
        for line in lines:
            story.append(Paragraph(_p(line), styles["body"]))
...
def build_pdf(compound_name, assessor, framework_note) -> bytes:
    ...
    story.append(Paragraph(f"<b>Compound:</b> {_p(compound_name)}", styles["body"]))
    story.append(Paragraph(f"<b>Prepared by:</b> {_p(assessor)}", styles["body"]))
    story.append(Paragraph(f"<b>Generated:</b> {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}", styles["body"]))
    story.append(Paragraph(f"<b>Framework note:</b> {_p(framework_note)}", styles["body"]))
    ...
    lines = []
    if woe:
        lines.append(f"Positive assays: {woe['n_positive']} / {woe['n_total']} usable")
        if woe.get("n_excluded"):
            lines.append(f"{woe['n_excluded']} assay result(s) excluded as borderline/inconclusive/invalid")
    ...
    if ra:
        ...
        if ra.get("potency_note"):
            lines.append(ra["potency_note"])
    ...
    if potency:
        if potency["path"] == "A":
            lines.append("Path: LLNA EC3-based")
...
def render_dossier_module():
    ...
    st.download_button(
        ...
        data=pdf_bytes,
        file_name=f"skin_sens_summary_{_safe_filename(compound_name)}.pdf",
