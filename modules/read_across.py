...
# Reuse the exact same SMARTS alert definitions as the Structure tab, rather
# than duplicating them -- one definition, used in two places.
try:
    from modules.structure import STRUCTURAL_ALERTS
except ImportError:
    STRUCTURAL_ALERTS = {}
...
# uses compounds that do NOT appear in any of the three real bundled sets
# below, so switching between lists never shows the same compound twice
# under different sets (it did until this was fixed: isopropanol, glycerol,
# lactic acid, and DNCB used to be duplicated across the demo set and the
# real sets — same molecules, harmless to any calculation since the app
# never merges the demo set with the real ones, but confusing to browse).
...
def _alert_vector(mol):
    """Boolean vector over STRUCTURAL_ALERTS, in a fixed key order, for one molecule."""
    return [
        bool(Chem.MolFromSmarts(smarts) is not None and mol.HasSubstructMatch(Chem.MolFromSmarts(smarts)))
        for smarts in STRUCTURAL_ALERTS.values()
    ]


def _alert_similarity(vec_a, vec_b) -> float:
    """Jaccard similarity over triggered alerts only (both-absent pairs don't count
    as agreement -- two totally different molecules that both happen to have zero
    alerts shouldn't score as 'similar' on this basis). Returns 0.0, not 1.0, when
    neither molecule triggers any alert, since there is no shared reactive signal
    to report.

    VALIDATED AND FOUND HARMFUL as implemented: see validate_read_across.py's
    "combined" mode results. With this dataset, 68% of sensitizers and 65% of
    non-sensitizers trigger zero of the 8 SMARTS alerts in STRUCTURAL_ALERTS, so
    most non-sensitizer/non-sensitizer pairs get this function's 0.0 floor, while
    the minority of sensitizer/sensitizer pairs that do share a triggered alert
    get boosted -- collapsing specificity to 0.000 across every tested (k, cutoff,
    alpha) combination. Do not wire this into the main app UI as a live option
    until a revised design is independently re-validated and actually helps;
    kept here, tested, and documented so a future attempt starts from a known
    failure mode instead of repeating it blindly.
    """
    intersection = sum(1 for a, b in zip(vec_a, vec_b) if a and b)
    union = sum(1 for a, b in zip(vec_a, vec_b) if a or b)
    return (intersection / union) if union else 0.0


def _combined_score(tanimoto: float, alert_sim: float, alpha: float) -> float:
    """Weighted blend of fingerprint similarity and structural-alert overlap.
    alpha=1.0 reduces to pure Tanimoto (the original behavior); alpha=0.0 would
    rely on alert overlap alone. Not assumed better than pure Tanimoto -- see
    validate_read_across.py for a head-to-head comparison before trusting either."""
    return alpha * tanimoto + (1 - alpha) * alert_sim
...
def compute_read_across(target_smiles: str, ref_df: pd.DataFrame, k: int = 3, domain_cutoff: float = 0.4,
    ...
    target_alerts = _alert_vector(target_mol) if similarity_mode == "combined" else None
    ...
    for _, r in ref_df.iterrows():
        ...
        if similarity_mode == "combined":
            alert_sim = _alert_similarity(target_alerts, _alert_vector(ref_mol))
            sim = _combined_score(tanimoto, alert_sim, alpha)
        else:
            alert_sim = None
            sim = tanimoto
...
def render_read_across_module():
    ...
    if ref_choice.startswith("Combined"):
        ...
        with st.expander("📖 Source & important caveats for this reference set", expanded=True):
            st.markdown(
                "- **Source:** the same ECVAM DPRA Validation Study Report as the ECVAM/JRC 24-compound "
                "set above, but a different table (Table 7: the chemicals used to qualify each transfer "
                "laboratory before the coded blind-testing phase). Distinct compounds from the main 24.\n"
                "- **Added deliberately to improve non-sensitizer balance**: the combined set was skewed "
                "toward sensitizers (33 positive vs 15 negative before this set was added); this set "
                "contributes 2 more real non-sensitizers (1-butanol, 6-methylcoumarin) plus 4 more "
                "sensitizers spanning moderate-to-weak potency.\n"
                "- This report gives only qualitative in-vivo potency (strong/moderate/weak/non-sensitizer), "
                "not numeric LLNA EC3 values, for this particular table — so ec3_pct is left blank here.\n"
                "- 6-Methylcoumarin is a real example of cross-source discordance: this validation study "
                "used it as a non-sensitizer, while some commercial safety data sheets list a precautionary "
